...
THREE.AMFLoader.prototype = {
	...
	parse: function ( data ) {
		...
		function loadDocumentScale( node ) {
			...
			for ( var i = 0; i < node.documentElement.attributes.length; i++ ) {
				if ( node.documentElement.attributes[i].name == 'unit' ) {
					unit = node.documentElement.attributes[i].value;
				}
			}
		...
		function loadMeshVolume( node ) {
			...
			for ( var i = 0; i < node.attributes.length; i++ ) {
				if ( node.attributes[i].name == 'materialid' ) {
					volume.materialId = node.attributes[i].value;
				}
			}
			...
			while ( currVolumeNode ) {
				...
				if ( currVolumeNode.nodeName === "metadata" ) {
					...
					volume.triangles.push( parseInt( v1 ) );
					volume.triangles.push( parseInt( v2 ) );
					volume.triangles.push( parseInt( v3 ) );
		...
		function loadObject( node ) {
			...
			while ( currObjNode ) {
				if ( currObjNode.nodeName === "metadata" ) {
					...
					for ( var i = 0; i < currObjNode.attributes.length; i++ ) {
						if ( currObjNode.attributes[i].name == 'type' && currObjNode.attributes[i].value == 'name' ) {
							loadedObject.name =  currObjNode.attributes[i].ownerElement.firstChild.data;
						}
					}
		...
		for ( var id in amfObjects ) {
			...
			for ( var i = 0; i < meshes.length; i ++ ) {
				...
				var vertices = new THREE.BufferAttribute( new Float32Array( meshVertices ), 3 );
				var meshNormals = null;
				var normals = null;

				if ( mesh.normals.length ) {

					meshNormals = new Float32Array( mesh.normals );
					normals = new THREE.BufferAttribute( new Float32Array( meshNormals ), 3 );
				...
				for ( var j = 0; j < volumes.length; j ++ ) {
					...
					var max = 0;
					var remixed_vertices = [];

					for ( var k = 0; k < volume.triangles.length / 3; k++ ) {
						var idx = k * 3,
							a = volume.triangles[ idx ] * 3,
							ax = mesh.vertices[ a ],
							ay = mesh.vertices[ a + 1 ],
							az = mesh.vertices[ a + 2 ],
							b = volume.triangles[ idx + 1 ] * 3,
							bx = mesh.vertices[ b ],
							by = mesh.vertices[ b + 1 ],
							bz = mesh.vertices[ b + 2 ],
							c = volume.triangles[ idx + 2 ] * 3,
							cx = mesh.vertices[ c ],
							cy = mesh.vertices[ c + 1 ],
							cz = mesh.vertices[ c + 2 ];

						remixed_vertices.push(
							ax, ay, az,
							bx, by, bz,
							cx, cy, cz
						);
					...
					vertices = new THREE.BufferAttribute( new Float32Array( remixed_vertices ), 3 );

					//newGeometry.setIndex( new THREE.BufferAttribute( indexes, 1 ) );
					newGeometry.addAttribute( 'position', vertices );
